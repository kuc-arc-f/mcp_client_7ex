...
${text} , ${num} JPYの購入を連絡します。`;
    const sendTo = import.meta.env.VITE_GOOGLE_MAIL_SENDTO;
    try{
...
async function exeuteSend(body: string, subject: string, sendTo: string){
  try {
    ...
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: import.meta.env.VITE_GOOGLE_MAIL_USER,     // 送信者のメールアドレス
        pass: import.meta.env.VITE_GOOGLE_MAIL_PASSWORD  // 送信者のメールパスワードまたはアプリパスワード
      }
    });
    // メールのオプション
    const mailOptions = {
      from: import.meta.env.VITE_GOOGLE_MAIL_USER,
      to: sendTo,
      subject: subject,
      text: body,
    };
